(function() {
    ...
    if (window.location.href.indexOf("MyZolog") !== -1 && window.location.href.indexOf("Login") === -1) {
        ...
        window.addEventListener('load', function() {
			...
			btnDebug.onclick = function() {
			  ...
			  var lrDoc = document.getElementById('iFrameMain') ? document.getElementById('iFrameMain').contentDocument : null;
			  var lrInner = lrDoc ? lrDoc.getElementById('iframeNewSummary') : null;
			  var lrInnerDoc = lrInner ? lrInner.contentDocument : null;
			  if (lrInnerDoc) {
					var panel = lrInnerDoc.getElementById('ddlLeaveTypePanel') || lrInnerDoc.getElementById('ddlLeaveTypeDropDown');
					if (panel) {
						var inputEl = lrInnerDoc.getElementById('ddlLeaveType_Input');
						var cs2 = lrInnerDoc.defaultView.getComputedStyle(inputEl);
						var rect2 = inputEl.getBoundingClientRect();
						out += 'INPUT: inline style="' + inputEl.getAttribute('style') + '" | computed width=' + cs2.width + ' actual width=' + rect2.width;
					}
					else {
						out += 'no ddlLeaveTypePanel/DropDown found';
					}
				}
    ...
    const preventZoomOnFocus = () => {
        const inputs = document.querySelectorAll('input[type="text"]:not(#ddlLeaveType_Input), input[type="password"]');
