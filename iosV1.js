    ...
    if (window.location.href.indexOf("MyZolog") !== -1 && window.location.href.indexOf("Login") === -1) {
        ...
        window.addEventListener('load', function() {
			...
			btnDebug.onclick = function() {
			  ...
			  if (lrInnerDoc) {
				var label = Array.from(lrInnerDoc.querySelectorAll('.formlabel')).find(function(el) {
					return el.textContent.trim().indexOf('Leave Type') === 0;
				});
				if (label) {
					out += 'Leave Type label inline style: "' + label.getAttribute('style') + '"\n';
					var labelTd = label.closest('td') === label ? label : label;
					out += 'labelTd tagName=' + label.tagName + '\n';
				} else {
					out += 'Leave Type label not found';
				}
			}
